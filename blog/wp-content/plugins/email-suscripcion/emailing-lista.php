...
add_menu_page( __( 'Emailing Subscription' .'' ), __( 'Emailing Subscription' .'' ),'edit_posts',  'emailing_list', 'emailing');
}
...
function emailing_install_data($name,$reference,$emaling) {
  ...
  }else*/if($emailrepet==0){
   $wpdb->insert( $table_name, array( 'time' => current_time('mysql'), 'reference' => $reference, 'email' => $emaling ) );
...
<form id="mailing-14" name="emailing" action="<?php echo $actual_link;?>#mailing-14" method="post" class="clear">
    <label>
    <input name="email_mail" id="email" type="email" class="email-field" placeholder="<?php _e( 'Email Address', 'emailing-list' ) ?>"/>
    </label>
    <input name="reference_mail" type="hidden" value="<?php the_title();?>">
    <label>
    <input class="" type="submit" name="emailing-send" value="<?php _e( 'sign up', 'emailing-list' ) ?>" >
    </label>
</form>
<?php
