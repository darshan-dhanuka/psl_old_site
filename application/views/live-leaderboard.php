...
<html lang="en">
<head>
  <title>Live Leaderboard of Pokersportsleague.com, All about PSL Live Leaderboard</title>
  <meta charset="utf-8">
...
<section class="online-board">
  <div class="container">
   ...
   <div class="col-xs-12">
 <h3 class="globlegreen">Keep track of the leaderboard to make sure you keep up with the competition.</h3></div>
   <div class="clr"></div>
...
function updateList(){
  ...
  var str = "";
   for(i = start ; i < count + start; i++ ){
      ...
      str += "<tr><td><span>" + result[i].rank + "</span></td><td><span>" + result[i].name + "</span></td><td><span>" + result[i].points + "</span></td><td><span>" + result[i].city + "</span></td></tr>" ;
    }
    ...
    if(page_number_str.length ==0)
    {
      $('.customBtn').hide();
      $('#no_record_div').text('No Record Found.');
    }
    else
    {
      $('.customBtn').show();
...
getLiveLeaderBoard('all');

 function getLiveLeaderBoard(city){
  $.post('/user/getLiveLeaderboardJson',{city:city})
  .done(function(res){
